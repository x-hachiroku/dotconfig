...
function rarx() {
    ...
    mkdir -p "$dst"
    rar x "${extra_args[@]}" "$archive" "$dst" && rm "$archive"
}
...
function 7zx() {
    ...
    local archive="${argv[-1]}"
    local dst="$(basename ${archive:r})"
    7z x "${extra_args[@]}" "$archive" -o"$dst" && rm "$archive"
}
...
function zipx() {
    ...
    local archive="${argv[-1]}"
    local dst="$(basename ${archive:r})"
    unzip "${extra_args[@]}" -d "$dst" "$archive" && rm "$archive"
}

alias parara='env_parallel --env rara --jobs 2 --progress -u rara {}'
