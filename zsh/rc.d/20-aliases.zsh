...
_rsync_cmd='rsync --archive --hard-links --human-readable --one-file-system --partial-dir=rsync --progress --verbose'
...
function scan() {
    scanimage -d "airscan:escl:HP_Scanner:http://10.6.6.200:80/eSCL/" --format=tiff --resolution 300 --mode Color | magick - scan.pdf
}

function termbl() {
  local bl_dir=(/sys/class/backlight/*(N[1]))
  local max_brightness=$(<"${bl_dir}/max_brightness")
  local target=$(( max_brightness * $1 / 10 ))

  echo "$target" > "${bl_dir}/brightness"
}
