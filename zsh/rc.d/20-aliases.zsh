...
function group_albums() {
    for flac in *.flac; do
        album="`ffprobe $flac 2>&1 | grep -i 'album ' | tr -s ' ' | cut -d ' ' -f 4-`"
        mkdir -p "$album"
        mv "$flac" "$album"
    done
}
...
function getrjimg() {
    local rj=$1
    local rjdir=$(( ( (${rj:2} + 999) / 1000 ) * 1000 ))
    local i=0
    wget https://web.archive.org/web/2022/https://img.dlsite.jp/modpub/images2/work/doujin/RJ${rjdir}/${rj}_img_main.webp -O folder.webp
    if [ $? -ne 0 ]; then
        wget https://web.archive.org/web/2022/https://img.dlsite.jp/modpub/images2/work/doujin/RJ${rjdir}/${rj}_img_main.jpg -O folder.jpg
        echo $?
        while [[ $? -eq 0 ]]; do
            i=$((i+1))
            wget https://web.archive.org/web/2022/https://img.dlsite.jp/modpub/images2/work/doujin/RJ${rjdir}/${rj}_img_smp${i}.jpg
        done
    else
        true # reset $?
        while [[ $? -eq 0 ]]; do
            i=$((i+1))
            wget https://web.archive.org/web/2022/https://img.dlsite.jp/modpub/images2/work/doujin/RJ${rjdir}/${rj}_img_smp${i}.webp
        done
    fi
}
